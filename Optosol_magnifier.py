    ...
    class Camera:
        ...
        def color_inversion(self, frame):
            ...
            if self.color == 2:
                ...
                self.cv2image = cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 115, 1)
            if self.color == 3:
                thresh, threading = cv2.threshold(frame, 150, 255, cv2.THRESH_BINARY)
                image = cv2.cvtColor(threading, cv2.COLOR_RGB2GRAY)
                maxIntensity = 255.0  # depends on dtype of image data
                ...
                self.cv2image = cv2.bitwise_not(run)
            if self.color == 4:
                ...
                thresh, self.cv2image = cv2.threshold(hsv_image, 200, 255, cv2.THRESH_BINARY)
            if self.color == 5:
                image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGBA)
                image = cv2.cvtColor(image, cv2.COLOR_RGB2YCR_CB)
                thresh, c = cv2.threshold(image, 100, 255, cv2.THRESH_BINARY)
                self.cv2image = cv2.bitwise_not(c)
        ...
        def show(self):
            while True:
                ...
                key = cv2.waitKey(1)
                if key == ord('7') or key == ord('7'):
                    # q : close
                    self.release()
                    cv2.destroyAllWindows()
                    break

                elif key == ord('1') or key == ord('1'):
                    # z : zoom - in
                    self.zoom_in()

                elif key == ord('2') or key == ord('2'):
                    # x : zoom - out
                    self.zoom_out()

                elif key == ord('3') or key == ord('3'):
                    self.change()

                elif key == ord('4') or key == ord('4'):
                    self.change_back()

                elif key == ord('5') or key == ord('5'):
                    # v : zoom original frame
                    self.touch_init()
                elif key == ord('6') or key == ord('6'):
                    self.save_picture()
